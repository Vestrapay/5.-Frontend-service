...
const Transfer = () => {
    ...
    useEffect(() => {
        setShowActualData(data?.filter((each: any) => each.paymentType == "TRANSFER"))
    }, [data])
                    ...
                    {actualData && actualData?.length < 1 ?
                        <>
                            <div className="w-full lg:w-2/3 text-black text-lg sm:text-5xl font-bold font-['Nunito'] sm:p-5">
                                You have not made any transfers in the last 30 days.
                            </div>
                            <div className="w-full lg:w-2/3 text-black text-base sm:text-xl font-bold font-['Nunito'] sm:p-5">
                                Make a bank transfer now.
                            </div>
                            <DefaultButton
                                icon={<BsPlus size={25} />}
                                labelText="Pay with Transfer"
