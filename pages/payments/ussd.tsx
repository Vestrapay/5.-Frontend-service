...
const Transfer = () => {
    ...
    useEffect(() => {
        setShowActualData(data?.filter((each: any) => each.paymentType == "USSD"))
    }, [data])
    ...
    return (
        <>
            <DashboardLayout>
                ...
                <main className="relative flex flex-1 mt-5 flex-col px-10 pb-4 h-[90%] w-full overflow-x-visible transition-all duration-300 ease-in-out px-10 sm:px-8 pb-10 h-full">
                    <nav>
                        <TransactionsNavbar apiRef={apiRef} data={actualData} name="USSD" setShowDelete={() => setShowDelete(true)} />
                    </nav>
                    {actualData && actualData?.length < 1 ?
                        <>
                            <div className="w-full lg:w-2/3 text-black text-lg sm:text-5xl font-bold font-['Nunito'] sm:p-5">
                            You have not made any transfers using USSD in the last 30 days.
                            </div>
