...
const fetchDisputesData = (pageNo: any, pageSize: any, search: string) => {
    ...
    const { isLoading, isError, error, isSuccess, data, refetch } = useQuery(
        ["ADMIN_DISPUTE_LIST_DATA", "DISPUTE", pageNo], () => func(),
        {
...
const updateDisputesController = (data: any) => {
    ...
    useEffect(() => {
        setState({
            ...
            merchantId: state?.merchantId || data?.merchantId || "",
            disputeId: data?.uuid || state?.disputeId || "",
            reason: state?.reason || data?.reason || "",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await apiCall({
                ...
                successDetails: { title: "Successful", text: "Logged Dispute updated successfully.", icon: "" },
                errorAction: (err?: any) => {
