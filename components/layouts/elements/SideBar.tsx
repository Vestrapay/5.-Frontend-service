...
function SideBar() {
  ...
  return (
    <>
      {/* Large Side bar */}
      <aside className="hidden md:flex w-[30%] min-h-[100vh] h-[100%] pt-5 pb-10 xl:h-full bg-white lg:drop-shadow-lg drop-shadow-lg z-50 lg:inset-x-auto inset-x-0 lg:inset-y-0 | self-start top-0 w-full max-w-[230px]  flex transition-all duration-300 ease-in-out fixed s">
        <div className=" w-full flex flex-col gap-10  justify-between overflow-y-auto text-slate-500 font-medium">
          <div className=" w-full flex flex-col gap-1 text-slate-500 font-medium">
            ...
            <div className="flex flex-col gap-2 px-8">
              ...
              isSubCategorySelected ? (
                <div className="relative text-base">
                  ...
                  <p className="text-selected text-base my-2">
                    {selectedSubMenuItem.name}
                  </p>
                  {selectedSubMenuItem?.subMenuItems?.map(
                    (item: SubMenuItems, index: number) => (
                      <div key={index} className="relative text-base">
                        {
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                <p className="my-2 w-full whitespace-nowrap">
                                  {item?.name}
                                </p>
                              </div>
                            </ActiveLinks>
                ...
                sidebarItems?.map((item: MenuItems | any, index: number) => (
                  <div key={index} className="relative text-base">
                    {activeLink === item.route && (
                      <p className="absolute -left-8 w-16 h-full -top-4 bottom-0 bg-gradient-to-r from-indigo-900 to-transparent opacity-60 " />
                    )}
                    {
                      <>
                        <ActiveLinks
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              <p className="font-300 text-base my-3">
                                {item?.name}
                              </p>
                            </motion.div>
                          </div>
            ...
            <Link
              href="https://documenter.getpostman.com/view/8528336/2s9YythgCF#42536433-0ce7-4bcb-b80a-9b219800beb7"
              passHref={true}
            >
              <motion.div
                whileHover={{ scale: 1.1 }}
          ...
          <div className=" flex flex-col justify-center items-start  bottom-8 ml-5">
            ...
            <div className="flex gap-2 items-start">
              <div className="flex justify-center items-center rounded-xl p-1 bg-slate-100  cursor-pointer hover:scale-105 transition-all duration-300 ease-in-out">
                <UserCircle
                  className={"w-7 h-7 text-unselected rounded-full "}
                />
              </div>
              ...
              <div className="flex flex-col justify-center items-start">
                ...
                >
                  ...
                  <LogoutOpen
                    ...
                    className="text-selected cursor-pointer hover:scale-105 transition-all duration-300 ease-in-out hover:text-ultraMarine "
                  />
                </div>
              </div>
            </div>
      ...
      {/* Small Side bar */}
      <div
        className={`w-full md:hidden absolute justify-between flex my-3 px-5`}
      >
        <Image src={Logo} alt="vestrapay" width={35} height={35} />

        <div
          className="w-8 h-8 md:hidden block bg-white rounded-full cursor-pointer"
      ...
      <MigrateToProd
        show={isMigrate}
        setShow={() => setIsMigrate(!isMigrate)}
      />
    </>
  );
}
