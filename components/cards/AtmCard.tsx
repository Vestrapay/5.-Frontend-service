...
const AtmCard = ({
    ...
    return (
        <div
            className={`flex relative w-full mx-10 lg:mx-0 lg:w-[750px] h-[196px] max-w-[400px] ${isActivated ? "bg-slate-400" : "bg-selected"} rounded-xl flex-col p-5 drop-shadow-md`}>
            ...
            <div className="flex justify-between items-center text-white my-6">
                <div className="flex flex-col">
                    <p className="text-[7px] m-0">Merchant Name</p>
                    <p className="text-white text-xs mt-1">{cardHolder}</p>
                </div>
                {/*<div>*/}
                {/*    <p className="text-[7px] m-0">Login date</p>*/}
                {/*    <p className="text-white text-xs mt-1">{dateTime}</p>*/}
                {/*</div>*/}
                {
                    cardProvider === "visa"
                        ? <RiVisaLine size={30}/>
